...
def dados_paciente(request, id):
    ...
    if request.method == "GET":
        ...
        if not peso.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de peso válida')
            return redirect('/dados_paciente/')
        if not altura.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de altura válida')
            return redirect('/dados_paciente/')
        if not gordura.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de gordura válida')
            return redirect('/dados_paciente/')
        if not musculo.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de musculo válida')
            return redirect('/dados_paciente/')
        if not hdl.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de hdl válida')
            return redirect('/dados_paciente/')
        if not ldl.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de ldl válida')
            return redirect('/dados_paciente/')
        if not colesterol_total.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de colesterol total válida')
            return redirect('/dados_paciente/')
        if not triglicerídios.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma medida de triglicerídios válida')
            return redirect('/dados_paciente/')
